...
public class CloudVision {
    ...
    public CloudVision (Context context) {
    ...
    private static class LableDetectionTask extends AsyncTask<Object, Void, String> {
        private final WeakReference<OnboardingActivity> mActivityWeakReference;
        private Vision.Images.Annotate mRequest;

        LableDetectionTask(OnboardingActivity activity, Vision.Images.Annotate annotate) {
    ...
    private void callCloudVision(final Bitmap bitmap) {
        ...
        try {
            AsyncTask<Object, Void, String> labelDetectionTask = new LableDetectionTask((OnboardingActivity) context, prepareAnnotationRequest(bitmap));
    ...
    private static void sendResponseText(BatchAnnotateImagesResponse response) {
        if(response.getResponses().get(0).getFullTextAnnotation() != null) {
            EventBus.getDefault().post(new OcrTextResult(response.getResponses().get(0).getFullTextAnnotation().getText()));
        } else {
            EventBus.getDefault().post(new OcrTextResult(resources.getString(R.string.no_results)));
        }
    }
}
